...
function renderEdge(vnode, co = false) {
    ...
    let labelPositionLongitudinal = vnode.attributes.labelPositionLongitudinal || 'middle'
    ...
    let pLongitudinal = ({middle: '', nearstart: " near start", nearend: ' near end'})[labelPositionLongitudinal]
    let [w1, w2] = vnode.attributes.value != null
        && needWrapChars.some(c => vnode.attributes.value.includes(c))
        ? ['{', '}'] : ['', '']
    let valueArg = vnode.attributes.value != null ? `"${w1}${vnode.attributes.value}${w2}"${p}${pLongitudinal}` : null
